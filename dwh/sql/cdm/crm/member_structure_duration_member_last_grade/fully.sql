        ...
        duration_member_last_grade AS (
            ...
            INNER JOIN ods_crm.order_info oi ON mgl.member_no = oi.member_no
                ...
                AND oi.order_deal_time >= date_trunc(
                    'month', DATE('{current_date}') + INTERVAL '-{computing_duration}' MONTH)
                AND oi.order_deal_time < date_trunc(
            ...
            WHERE 1 > (SELECT COUNT(*)
                ...
                AND date_diff('day', mgl.grade_change_time, date_trunc('month', DATE('{current_date}') + INTERVAL '-2' MONTH )) > 0
