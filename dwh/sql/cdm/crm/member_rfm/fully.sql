INSERT INTO cdm_crm.member_rfm (
    ...
    WITH
        ...
        order_info_range AS (
            ...
            AND order_deal_time >= date_trunc('month', DATE('{current_date}') + INTERVAL '-{computing_duration}' YEAR) AND order_deal_time < date_trunc('month', DATE('{current_date}'))
        ),
        ...
        member_recency_frequency_order_count_min_max_order_deal_date_n_arr_monetary_total_n_per_order AS (
            ...
            date_diff('day', DATE(max(oi.order_deal_time)),
                        date_trunc('month', DATE('{current_date}')) + INTERVAL '-1' DAY) recency,
        ...
        member_last_log AS (
            ...
            WHERE 1 > (SELECT count(*)
                ...
                AND date_diff('day', mgl.grade_change_time, date_trunc('month', DATE('{current_date}'))) > 0
