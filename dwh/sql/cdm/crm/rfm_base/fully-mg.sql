INSERT INTO cdm_crm.rfm_base (
    ...
    WHERE monetary_per_order > cast(json_extract(json_parse('{rfm_json}'), '$.h_greater_than') AS BIGINT)
            AND monetary_per_order <= cast(json_extract(json_parse('{rfm_json}'), '$.h_not_greater_than') AS BIGINT)
