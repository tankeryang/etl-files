INSERT INTO cdm_crm.member_structure_asset (
    ...
    WITH
        ...
        grade_new_old_member_recency AS (
            ...
            GROUP BY computing_until_month, computing_duration, store_region, grade_code, member_type, recency
        ),
        ...
        grade_new_old_member AS (
            ...
            GROUP BY computing_until_month, computing_duration, store_region, grade_code, member_type
        ),
        --全渠道、全国区域下各等级的会员统计
        grade_member AS (
            SELECT
            ...
            GROUP BY computing_until_month, computing_duration, store_region, grade_code
        ),
        --全渠道、全国区域会员
        total_member AS (
            SELECT
            ...
            LOCALTIMESTAMP               AS create_time
            FROM grade_member
            GROUP BY computing_until_month, computing_duration, store_region
        ),
        --非会员
        not_member AS (
            SELECT
        ...
        potential_member_reg_source AS (
            ...
            GROUP BY computing_until_month, computing_duration, store_region, reg_source
        ),
        --潜在客户
        potential_member AS (
            SELECT
