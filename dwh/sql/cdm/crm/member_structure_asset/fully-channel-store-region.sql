INSERT INTO cdm_crm.member_structure_asset (
    ...
    WITH
        ...
        grade_new_old_member_recency AS (
            ...
            GROUP BY computing_until_month, computing_duration, channel_type, store_region, grade_code, member_type, recency
        ),
        ...
        grade_new_old_member AS (
            ...
            GROUP BY computing_until_month, computing_duration, channel_type, store_region, grade_code, member_type
        ),
        --全渠道、全国区域下各等级的会员统计
        grade_member AS (
            SELECT
            ...
            GROUP BY computing_until_month, computing_duration, channel_type, store_region, grade_code
        ),
        --全渠道、全国区域会员
        total_member AS (
            SELECT
            ...
            LOCALTIMESTAMP               AS create_time
            FROM grade_member
            GROUP BY computing_until_month, computing_duration, channel_type, store_region
        ),
        --非会员
        not_member AS (
            SELECT
        ...
        purchased_member AS (
            ...
            total_member.channel_type,
            NULL                                                                      AS sales_area,
            total_member.store_region,
            'PURCHASED'                                                               AS purchase_type,
            NULL                                                                      AS vip_type,
            NULL                                                                      AS reg_source,
            NULL                                                                      AS grade_code,
            ...
            LOCALTIMESTAMP                                                            AS create_time
            FROM total_member
            LEFT JOIN not_member
                ON total_member.computing_until_month = not_member.computing_until_month AND
                total_member.computing_duration = not_member.computing_duration AND
                total_member.channel_type = not_member.channel_type AND
                total_member.store_region = not_member.store_region
        ),
        ...
        potential_member_reg_source AS (
            ...
            GROUP BY computing_until_month, computing_duration, channel_type, store_region, reg_source
        ),
        --潜在客户
        potential_member AS (
            SELECT
        ...
        total_member_asset AS (
            ...
            purchased_member.channel_type,
            NULL                                                                      AS sales_area,
            purchased_member.store_region,
            ...
            LEFT JOIN potential_member
                ...
                purchased_member.computing_duration = potential_member.computing_duration AND
                purchased_member.channel_type = potential_member.channel_type AND
                purchased_member.store_region = potential_member.store_region
        )
    SELECT *
