INSERT INTO dwd_crm.order_coupon_info
    ...
    WHERE cu_t.create_time > (SELECT MAX(create_time) FROM dwd_crm.order_coupon_info)
        AND cu_t.create_time < DATE_PARSE(DATE_FORMAT(localtimestamp, '%Y-%m-%d 00:00:00'), '%Y-%m-%d %T')
    GROUP BY
        oit.outer_order_no,
        cu_t.coupon_category,
        cu_t.coupon_no_array,
