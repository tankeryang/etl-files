...
CREATE TABLE cdm_crm.order_info_detail (
    ...
    is_member                        TINYINT         COMMENT '是否会员 - 1:会员, 0:非会员',
    is_new_member                    TINYINT         COMMENT '新老会员分析 - 1:新会员, 0:老会员',
    is_vip_member                    TINYINT         COMMENT '会员等级分析 - 1:VIP会员, 0:普通会员',
    is_normal_upgrade_member         TINYINT         COMMENT '会员升级类型(普通升VIP) - 1:升级, 0:未升级',
    is_upgrade_member                TINYINT         COMMENT '会员升级类型(直接升) - 1:升级, 0:未升级',
    member_register_type             VARCHAR         COMMENT '会员注册类型 - STORE, WEB',
    member_recruit_type              VARCHAR         COMMENT '招募会员-有消费会员类型 - NORMAL, VIP, UPGRADE',
    ...
    order_type                       TINYINT         COMMENT '订单标识(1:正价单/-1:退单/0:0价单 -- 用于统计单量, 退单要抵消)',
    ...
    order_deal_year_month            VARCHAR         COMMENT '订单交易年月',
    order_deal_year                  INTEGER         COMMENT '订单交易年份',
    order_deal_month                 INTEGER         COMMENT '订单建议月份',
