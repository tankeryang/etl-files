  ...
  WITH
    ...
    --***活跃新老客户-购买次数分组统计，客户数、成交额、人均成交额
      total_active_member_type_frequency_member AS (
        ...
        GROUP BY computing_until_month, computing_duration, channel_type, store_region, member_type, frequency_type
    ),
    ----活跃新老客户分组统计，总客户数、总成交额、总人均成交额
      total_active_member_type_member AS (
        ...
        GROUP BY computing_until_month, computing_duration, channel_type, store_region, member_type
    ),
    --活跃重复购买新老客户
    ...
    --***活跃-重复购买-新老客户统计，总客户数、总成交额、人均成交额、重复购买人数、重复购买金额、重复购买率
      total_active_n_repurchased_member_type_member AS (
        ...
        ORDER BY tamtm.computing_until_month, tamtm.computing_duration, tamtm.channel_type, tamtm.store_region
    ),
    ...
    --活跃老客户本月、季度、半年、年等的成交额
      member_structure_active_old_duration_monetary_per_member AS (
        ...
        GROUP BY computing_until_month, computing_duration, channel_type, store_region, member_no
    ),
    ...
    --***活跃老客户本月、季度、半年、年等之前的最近一次购买分布在上(及上上、上上上、...等)月、季度、半年、年等的哪个区间
      total_active_old_recency_type_member AS (
        ...
        GROUP BY computing_until_month, computing_duration, channel_type, store_region, recency_type
    )
