  ...
  WITH
    ...
    --***活跃新老客户-购买次数分组统计，客户数、成交额、人均成交额
      total_active_member_type_frequency_member AS (
        ...
        GROUP BY computing_until_month, computing_duration, channel_type, sales_area, store_region, member_type,
          frequency_type
    ),
    ...
    --活跃老客户本月、季度、半年、年等的成交额
      member_structure_active_old_duration_monetary_per_member AS (
        ...
        GROUP BY computing_until_month, computing_duration, channel_type, sales_area, store_region, member_no
    ),
    ...
    --***活跃老客户本月、季度、半年、年等之前的最近一次购买分布在上(及上上、上上上、...等)月、季度、半年、年等的哪个区间
      total_active_old_recency_type_member AS (
        ...
        GROUP BY computing_until_month, computing_duration, channel_type, sales_area, store_region, recency_type
    )
