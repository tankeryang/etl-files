  ...
  WITH
    --全渠道、全国区域下各等级下的新老会员下的recency统计
      grade_new_old_member_recency AS (
        ...
        GROUP BY computing_until_month, computing_duration, sales_area, store_region, grade_code, member_type, recency
    ),
    ...
    --全渠道、全国区域下各等级下的新老会员统计
      grade_new_old_member AS (
        ...
        GROUP BY computing_until_month, computing_duration, sales_area, store_region, grade_code, member_type
    ),
    --全渠道、全国区域下各等级的会员统计
      grade_member AS (
        SELECT
        ...
        GROUP BY computing_until_month, computing_duration, sales_area, store_region, grade_code
    ),
    --全渠道、全国区域会员
      total_member AS (
        SELECT
        ...
        FROM total_member
          LEFT JOIN not_member
            ON total_member.computing_until_month = not_member.computing_until_month AND
               ...
               total_member.sales_area = not_member.sales_area AND
    ...
    --潜在客户-注册来源
      potential_member_reg_source AS (
        ...
        GROUP BY computing_until_month, computing_duration, sales_area, store_region, reg_source
    ),
    --潜在客户
      potential_member AS (
        SELECT
        ...
        FROM purchased_member
          LEFT JOIN potential_member
            ON purchased_member.computing_until_month = potential_member.computing_until_month AND
               ...
               purchased_member.sales_area = potential_member.sales_area AND
