INSERT INTO cdm_crm.daliy_report_country_day
    SELECT
        sales_area,
        city,
        store_code,
        ...
        -- 会员基数(累计)
        member_amount,
        IF(past_12_month_remain_member_amount IS NOT NULL, past_12_month_remain_member_amount, 0),
        second_trade_rate,
    ...
    FROM (
        SELECT
            ...
            -- lmr.past_12_month_remain_member_amount  AS past_12_month_remain_member_amount,
            -- 回头率
            cast(IF (
        ...
        LEFT JOIN (
            SELECT
                drsdb.country,
                drsdb.member_type,
                cast(sum(drsdb.sales_amount) AS DECIMAL(38, 2))  last_year_same_time_sales_amount,
