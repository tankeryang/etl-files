...
FROM (
    SELECT DISTINCT
        sales_area, order_channel, sales_mode, store_type, store_level, channel_type, member_type
    FROM cdm_crm.member_analyse_type_label
) matl2

LEFT JOIN (
    SELECT sales_area, order_channel, sales_mode, store_type, store_level, channel_type, member_type,
    ...
    GROUP BY sales_area, order_channel, sales_mode, store_type, store_level, channel_type, member_type
) sm2
ON matl2.sales_area = sm2.sales_area
...
LEFT JOIN (
    SELECT sales_area, order_channel, sales_mode, store_type, store_level, channel_type,
    sum(mab.order_fact_amount) *1.0 / 10000 AS ttsi
    FROM mab
    WHERE date(mab.order_deal_time) <= date('2018-08-10')
    AND date(mab.order_deal_time) >= date('2018-01-01')
    GROUP BY sales_area, order_channel, sales_mode, store_type, store_level, channel_type
) smtt
ON matl2.sales_area = smtt.sales_area
...
ON matl2.sales_area = lyst2.sales_area
...
FROM (
    SELECT DISTINCT
    sales_area, order_channel, sales_mode, store_type, store_level, channel_type, member_nowbefore_type
    FROM cdm_crm.member_analyse_type_label
) matl

LEFT JOIN (
    SELECT sales_area, order_channel, sales_mode, store_type, store_level, channel_type, member_nowbefore_type,
    sum(mab.order_fact_amount) * 1.0 / 10000 AS si,
    ...
    WHERE date(mab.order_deal_time) <= date('2018-08-10')
    AND date(mab.order_deal_time) >= date('2018-01-01')
    GROUP BY sales_area, order_channel, sales_mode, store_type, store_level, channel_type, member_nowbefore_type
) sm
    ON matl.sales_area = sm.sales_area
...
ON matl.sales_area = smtt.sales_area
...
ON matl.sales_area = lyst.sales_area
