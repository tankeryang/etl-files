INSERT INTO ads_crm.member_analyse_income_total_all
    WITH tt AS (
        ...
        SELECT
            ...
            ss.{zone},
            ss.member_type,
            IF (ss.order_channel IS NULL, '全部', ss.order_channel) AS order_channel,
            IF (ss.sales_mode IS NULL, '全部', ss.sales_mode) AS sales_mode,
            IF (ss.store_type IS NULL, '全部', ss.store_type) AS store_type,
            IF (ss.store_level IS NULL, '全部', ss.store_level) AS store_level,
            IF (ss.channel_type IS NULL, '全部', ss.channel_type) AS channel_type,
        ...
        SELECT
            ...
            ss_l.{zone},
            ss_l.member_type,
            IF (ss_l.order_channel IS NULL, '全部', ss_l.order_channel) AS order_channel,
            IF (ss_l.sales_mode IS NULL, '全部', ss_l.sales_mode) AS sales_mode,
            IF (ss_l.store_type IS NULL, '全部', ss_l.store_type) AS store_type,
            IF (ss_l.store_level IS NULL, '全部', ss_l.store_level) AS store_level,
            IF (ss_l.channel_type IS NULL, '全部', ss_l.channel_type) AS channel_type,
