INSERT INTO ads_crm.member_analyse_income_member_new_old_zone_monthly
    WITH tt AS (
        SELECT
            brand_name AS brand,
        ...
        GROUP BY DISTINCT
            brand_name, {zone}, member_newold_type, year, month,
            CUBE (order_channel, sales_mode, store_type, store_level, channel_type)
    ), tmp AS (
        SELECT DISTINCT
        ...
        GROUP BY DISTINCT
            f.brand_name, f.{zone}, f.member_newold_type, f.year, f.month,
